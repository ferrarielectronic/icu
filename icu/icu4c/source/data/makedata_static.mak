...
ICU_SLIB_TARGET=$(LIB_OUTPUT)\sicudt.lib
...
!MESSAGE ICU data LIB_OUTPUT path is $(LIB_OUTPUT)
...
"$(ICU_LIB_TARGET)" : $(COMMON_ICUDATA_DEPENDENCIES) $(COREDATA_TS)
	...
	@if not exist "$(LIB_OUTPUT)" mkdir "$(LIB_OUTPUT)"
	@echo Copy static data library ...
	copy "$(ICUBLD_PKG)\s$(U_ICUDATA_NAME).lib" "$(ICU_SLIB_TARGET)"
	rem -@erase "$(U_ICUDATA_NAME).dll"
    @echo copy output to staging dir
	copy "$(ICU_SLIB_TARGET)" "$(STAGING_PREFIX)\$(CONFIGURATION)\$(PLATFORM)$(PLATFORMTOOLSET)\lib\*.*" /y
	copy "$(ICUTMP)\$(ICUPKG).dat" "$(ICUOUT)\$(U_ICUDATA_NAME)$(U_ICUDATA_ENDIAN_SUFFIX).dat"
	-@erase "$(ICUTMP)\$(ICUPKG).dat"
!ENDIF
